...
/**
 * Created by Amrane Ait Zeouay on 19-Sep-17.
 */

public class SpecialButton extends Button implements View.OnTouchListener {
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SpecialButton);
        if (typedArray == null) return;
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.SpecialButton_shadowEnabled) {
                isShadowEnabled = typedArray.getBoolean(attr, true); //Default is true
            } else if (attr == R.styleable.SpecialButton_buttonColor) {
                mButtonColor = typedArray.getColor(attr, ContextCompat.getColor(context, R.color.fbutton_default_color));
            } else if (attr == R.styleable.SpecialButton_shadowColor) {
            ...
            } else if (attr == R.styleable.SpecialButton_shadowHeight) {
                mShadowHeight = typedArray.getDimensionPixelSize(attr, R.dimen.fbutton_default_shadow_height);
            } else if (attr == R.styleable.SpecialButton_cornerRadius) {
                mCornerRadius = typedArray.getDimensionPixelSize(attr, R.dimen.fbutton_default_conner_radius);
            } else if (attr == R.styleable.SpecialButton_specialButtonText) {
                style = typedArray.getInt(R.styleable.CommonTextView_font_style, FontManager.Style.FILBERT_BRUSH);
            }
        }
        typedArray.recycle();
